...
func NewDiagnoseParser(tokStream TokenStream, prs ParseTable, maxErrors int, maxTime int, monitor Monitor) *DiagnoseParser {
	...
	self.list = make([]int, self.NUM_SYMBOLS+1)
	return self
}
...
func (self *DiagnoseParser) reallocateStacks() {
	...
	var stack_length int = old_stack_length + self.STACK_INCREMENT

	if len(self.stateStack) == 0 {
		self.stateStack = make([]int, stack_length)
		self.locationStack = make([]int, stack_length)
		self.tempStack = make([]int, stack_length)
		self.prevStack = make([]int, stack_length)
		self.nextStack = make([]int, stack_length)
		self.scopeIndex = make([]int, stack_length)
		self.scopePosition = make([]int, stack_length)
	} else {
		self.stateStack = arraycopy(self.stateStack, 0, make([]int, stack_length), 0, old_stack_length)
		self.locationStack = arraycopy(self.locationStack, 0, make([]int, stack_length), 0, old_stack_length)
		self.tempStack = arraycopy(self.tempStack, 0, make([]int, stack_length), 0, old_stack_length)
		self.prevStack = arraycopy(self.prevStack, 0, make([]int, stack_length), 0, old_stack_length)
		self.nextStack = arraycopy(self.nextStack, 0, make([]int, stack_length), 0, old_stack_length)
		self.scopeIndex = arraycopy(self.scopeIndex, 0, make([]int, stack_length), 0, old_stack_length)
		self.scopePosition = arraycopy(self.scopePosition, 0, make([]int, stack_length), 0, old_stack_length)
	}
	...
	for ; i < self.SCOPE_SIZE; i++ {
		...
		for ; action_index < action.size(); action_index++ {
			...
			if act != self.ERROR_ACTION {
				...
				for j = self.tempStackTop + 1; j >= (max_pos+1) &&
					self.inSymbol(self.tempStack[j]) == self.scopeRhs(k); j-- {
					k++
				}
				if j == max_pos {
					for j = max_pos; j >= 1 && self.inSymbol(stack[j]) == self.scopeRhs(k); j-- {
						k++
					}
				}
				//
				...
				if self.scopeRhs(k) == 0 && j < marked_pos { // match?
					...
					for j = self.scopeStateSet(i); stack[stack_position] != self.scopeState(j) &&
						self.scopeState(j) != 0; j++ {
					}
					//
					...
					if self.scopeState(j) != 0 { // state was found
						...
						// of recovery.
						//
						if self.tokStream.getKind(self.buffer[repair.bufferPosition]) == self.EOFT_SYMBOL &&
